...
function toggleinfo(e) {
    ...
    // get the height of tab pane based on contents
    $('#info .tab-pane').each(function(){
        var el = this;
        if( $(el).hasClass('active')){
            active_ht = getHeight($(this));
        }else{
            var el_ht = getHeight($(this));
            if ( el_ht > tab_ht){
                tab_ht = el_ht;
            }
        }
    ...
    // if a tab pane exists that is larger than the visible apply as minimum table height
    if(active_ht < tab_ht && tab_ht !== 0){
        $('.tab-pane').css('min-height', tab_ht + 'px');
    }else if( active_ht > tab_ht && tab_ht !== 0){
        $('.tab-pane').css('min-height', active_ht + 'px');
    }
...
function highlightMapFeature(lyr){
    ...
    if (id != 'Point'){
        ...
        } else if($('#preselect').hasClass('circle-cm') || $('#preselect').hasClass('crc-cm')) {
            $(iconElem).prepend('<div class="markerBox cm" style="border-color:'+hlColor+'"></div>');
            iconElem.id='selectedIcon';
        }else if($('#preselect').hasClass('circle-sm') || $('#preselect').hasClass('crc-sm')) {
            $(iconElem).prepend('<div class="markerBox sm" style="border-color:'+hlColor+'"></div>');
            iconElem.id='selectedIcon';
        }else if($('#preselect').hasClass('circle-md') || $('#preselect').hasClass('crc-md')) {
            $(iconElem).prepend('<div class="markerBox md" style="border-color:'+hlColor+'"></div>');
            iconElem.id='selectedIcon';
        }
        */
    }
...
//getHeight of elements that may be hidden
var getHeight = function(el) {
    
    var el_style      = el.css('display'),
        el_display    = el.css('display'),
        el_position   = el.css('position'),
        el_visibility = el.css('visibility');
        el_max_height = el.css('max-height').replace('px', '').replace('%', ''),
    ...
    wanted_height     = el.outerHeight();

    // remove the applied inline styles
    el.css('display', '');
    el.css('visibility', '');
    el.css('position', '');
